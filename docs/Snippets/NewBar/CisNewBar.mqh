...
bool CisNewBar::isNewBar(datetime newbar_time) {
   ...
   if(m_lastbar_time > newbar_time) {   // If new bar is older than last bar, print error message
...
int CisNewBar::isNewBar() {
   ...
   if(!SeriesInfoInteger(m_symbol, m_period, SERIES_LASTBAR_DATE, newbar_time)) {   // If request has failed, print error message:
